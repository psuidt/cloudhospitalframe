...
namespace EfwControls.HISControl.PrescriptionTmp.Controls
{
    ...
    public partial class PrescriptionTmpControl : UserControl, IPrescriptionControl, IPrescription
    {
        #region 许可证
        [Browsable(false)]
        ...
        {
            ComponentLicense lic = LicenseManager.Validate(typeof(PrescriptionTmpControl), this) as ComponentLicense;
            if (lic != null)
            {
                _isDemo = lic.IsDemo;
                if (_isDemo)
                {
                    MessageBox.Show("正确设置[PrescriptionTmpControl]控件的许可证，才能使用！");
                }
            }
        }
        ...
        public PrescriptionTmpControl(IContainer container)
        {
            container.Add(this);
            ...
            //初始化控制器
            controller = new PrescriptionControlController(this);
            panelFoot.Visible = false;    //模板不显示金额
            ...
            try
            {
                if (SinglePresPrint != null)
                {
                    if (gridPresDetail.CurrentCell != null)
                    {
                        ...
                        SinglePresPrint(CurrPatListId, (int)PrescriptionStyle, presNo);
                    }
                }
            }
            catch (Exception err)
        ...
        private void tSMnuISelfDrug_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                (controller as PrescriptionControlController).PresSelfDrug();
        ...
        private void tSMnuISaveAsMould_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        //先弹出界面输入模板名，再点击保存
                        SaveSetMeal setMeal = new SaveSetMeal();
                        setMeal.ShowDialog();
                        if (setMeal.IsOK)
                        {
                            controller.AsSavePresTemplate(setMeal.MealType, setMeal.MealName, (int)PrescriptionStyle, data);
                            MessageBoxEx.Show("另存模板成功!");
                        }
                    }
                }
            }
        ...
        public PrescriptionTmp.Controls.Entity.PresColor CellColor
        {
            set
            {
                if (value.colIndex == -1)
        ...
        public void PrescriptionLoadList(List<PrescriptionTmp.Controls.Entity.Prescription> data)
        {
    ...
    //单张处方打印
    public delegate void SinglePrescriptionPrint(int patListId,int presType,int presNo);
    //未收费费用清单打印
